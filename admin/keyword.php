...
define('MODULE_NAME', 'core-adm.keyword');
...
class Keyword extends CommonLib
{
	...
	function Keyword()
	{
		$this->_initEnv(MODULE_NAME, 'keyword', 0);

		if (FALSE == empty($this->_input['_POST']['id']) && TRUE == is_numeric($this->_input['_POST']['id'])) $this->keyword_id = intval($this->_input['_POST']['id']);
		elseif (FALSE == empty($this->_input['_GET']['id']) && TRUE == is_numeric($this->_input['_GET']['id'])) $this->keyword_id = intval($this->_input['_GET']['id']);

		if (FALSE == empty($this->_input['_POST']['action']))
		{
			switch ($this->_input['_POST']['action'])
			{
				case 'add' : $this->_addKeyword();
					break;
				case 'modify' : $this->_modifyKeyword();
					break;
				default : $this->_printMainPage();
					break;
			} 
		} elseif (FALSE == empty($this->_input['_GET']['action']) && 'delete' == $this->_input['_GET']['action']) $this->_deleteKeyword();
