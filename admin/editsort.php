...
class EditSort extends CommonLib
{
	...
	function EditSort()
	{
		...
		include_once("../{$this->_config['global']['langURL']}/editsort.php");
		$this->_lang['editsort'] = $lang;
		// 输入数据
		...
		elseif (FALSE == empty($_GET['id']) && TRUE == is_numeric($_GET['id'])) $this->sort_id = intval($_GET['id']);

		if (FALSE == empty($_GET['action']))
		{
			switch ($_GET['action'])
			{
				case 'add' : $this->_printAddModifyForm(); break;
				case 'edit' :
				...
				case 'delete':
					...
					break;
				case 'rank': $this->_printChangeSortRank(); break;
		...
		elseif (FALSE == empty($this->_input['_POST']['action']))
		{
			...
			{
				case 'addasort' : $this->_addSort(); break;
				case 'updasort' : $this->_modifySort(); break;
				case 'changerank' : $this->_changeSortRank(); break;
			}
		}
		else $this->_printSortList();
