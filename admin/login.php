...
define('MODULE_NAME', 'core-adm.login');
...
class Login extends CommonLib
{
	...
	function Login()
	{
		$this->_initEnv(MODULE_NAME, 'login', -1);

		if (FALSE == empty($this->_input['_GET']['rurl'])) $this->return_url = urldecode($this->_input['_GET']['rurl']);
	...
	function _loginHandler()
	{
		...
		if (FALSE == count($this->errors))
		{
			...
			else
			{
				...
				@session_write_close(); // The session data maybe lost ... :(
			} 
			$this->_dbop->freeResult();

			if (FALSE == count($this->errors))
			{
				if (FALSE == empty($this->return_url)) header("Location: $this->return_url");
				elseif (TRUE == empty($_SERVER['HTTP_REFERER'])) header("Location: frame.php");
