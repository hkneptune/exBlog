...
class Tags extends CommonLib
{
	...
	function Tags()
	{
		...
		include_once("../{$this->_config['global']['langURL']}/tags.php");
		$this->_lang['tags'] = $lang;
		// 输入数据
		...
		elseif (FALSE == empty($_GET['id']) && TRUE == is_numeric($_GET['id'])) $this->tag_id = intval($_GET['id']);
		else $this->tag_id = 0;
		...
		if (FALSE == empty($this->_input['_POST']['action']))
		{
			...
			{
				case 'add' : $this->_addTag(); break;
				case 'modify' : $this->_modifyTag(); break;
				default : $this->_printTagsList(); break;
			}
		}
		elseif (FALSE == empty($_GET['action']))
		{
			switch ($_GET['action'])
			{
				case 'delete' : $this->_deleteTag(); break;
				default : $this->_printTagsList(); break;
			}
		}
