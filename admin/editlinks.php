...
class EditLinks extends CommonLib
{
	...
	function EditLinks()
	{
		...
		include_once("../{$this->_config['global']['langURL']}/editlinks.php");
		$this->_lang['editlinks'] = $lang;
		// 输入数据
		...
		elseif (FALSE == empty($_GET['id']) && TRUE == is_numeric($_GET['id'])) $this->link_id = intval($_GET['id']);

		if (FALSE == empty($_GET['action']))
		{
			if ('add' == $_GET['action']) $this->_printEditLinks();
			elseif ('edit' == $_GET['action'] && FALSE == empty($this->link_id)) $this->_printEditLinks();
			elseif ('rank' == $_GET['action']) $this->_printLinksOrder();
			elseif ('delete' == $_GET['action'] && FALSE == empty($this->link_id)) $this->_deleteLinks($this->link_id);
		...
		elseif (FALSE == empty($this->_input['_POST']['action']))
		{
			...
			{
				case 'addalink' : $this->_addLink(); break;
				case 'edialink' : $this->_modifyLink(); break;
				default : $this->_printEditLinks(); break;
			}
		}
