 ...
 * Copyright (C) 2005, 2006  feeling <feeling@4ulm.org>
 *
 ...
 * @brief @~chinese MySQL 数据库操作工具
 * @author feeling <feeling@4ulm.org>
 * @date 2005-01-16 03:37:16
 * @note @~chinese 本工具集支持 MySQL / SQLite 库
 * @warning @~chinese 本工具集可能使用到了错误处理句柄，因此在使用之前请选择好你的错误处理方式
 * @todo @~chinese @li 扩展 close() 方法，加入对以持续连接方式建立的连接的支持
 **/
class Database
{
	/**
	 * @brief @~chinese 数据库服务器IP地址或主机名
	 **/
	var $host = "localhost";
	/**
	 * @brief @~chinese 数据库操作用户名
	 **/
	var $user = "root";
	/**
	 * @brief @~chinese 数据库操作用户名密码
	 **/
	var $passwd = "";
	/**
	 * @brief @~chinese 默认操作数据库名称
	 **/
	var $dbname = "";
	/**
	 * @brief @~chinese 数据库服务器TCP端口
	 **/
	var $port = 3306;
	/**
	 * @brief @~chinese 数据库服务器UNIX SOCKET文件位置
	 **/
	var $socket = "/tmp/mysql.sock";
	/**
	 * @brief @~chinese 是否使用持续连接@li TRUE ＝ 是。将加大服务器负载，但是可以尽量减少数据库服务器出现“连接数过多”的错误提示
	 *                                                                    @li FALSE ＝ 否。将减小服务器负载，但是对于并发数较多的站点可能会导致时常出现“连接数过多”的错误提示
	 **/
	var $pconnect = TRUE;
	/**
	 * @brief @~chinese 数据库中所有数据表名的前缀，可用于区别同一程序的多个实例
	 **/
	var $prefix = "";
	/**
	 * @brief @~chinese 数据库运行模式，可选值如下：@li mysql ＝ 数据库为MySQL，并计划使用PHP的MySQL库@li mysqli ＝ 数据库为 MySQL，并计划使用PHP的库
	 **/
	var $mode = "mysql";

	/**
	 * @internal
	 * @brief @~chinese 数据库连接ID
	 **/
	var $connect_id = 0;
	/**
	 * @internal
	 * @brief @~chinese 数据库查询ID
	 **/
	var $query_id = 0;
	/**
	 * @brief @~chinese 数据库查询结果
	 **/
	var $records = array();
	/**
	 * @brief @~chinese 数据库查询的最近错误记录
	 * @see Database::getError()
	 **/
	var $last_error = array();
	/**
	 * @brief @~chinese 数据库缺省字符集
	 **/
	var $charset = "DEFAULT";
	/**
	 * @brief @~chinese MySQL版本
	 **/
	var $version = "4.1.13a-nt";
	/**
	 * @brief @~chinese 是否支持自定义字符集
	 **/
	var $support_charset = FALSE;

	/**
	 * @brief @~chinese 构造器
	 * @param $options @~chinese 用于设置操作习惯的选项列表，其可用的索引名称为 "host", "user", "passwd", "dbname", "pconnect", "prefix", "socket", "port", "mode" 之一
	 **/
	...
	/**
	 * @brief @~chinese 连接指定的数据库服务器，如果指定了数据库名称，则在成功连接数据库服务器以后选定该数据库
	 * @param $host @~chinese 数据库服务器IP地址或主机名
	 * @param $user @~chinese 数据库操作用户名
	 * @param $passwd @~chinese 数据库操作用户名密码
	 * @param $dbname @~chinese 默认操作数据库名称
	 * @param $port @~chinese 数据库服务器TCP端口
	 * @param $socket @~chinese 数据库服务器UNIX SOCKET文件位置
	 * @return @~chinese 如果成功则返回连接ID，否则将错误信息标记为 E_USER_ERROR 并传递到错误处理句柄
	 **/
	...
	/**
	 * @brief @~chinese 获取指定连接的最后一次错误信息
	 * @param $type @~chinese 获取的错误信息类型，可以为以下选项之一或者使用逗号","分隔的选项列表@li number 错误号@li message 错误描述
	 * @param $connect_id @~chinese 指定需要获取的连接ID。如果为空，则使用最近一次的连接ID
	 * @return @~chinese 如果 $type 为一个选项，则返回该选项对应的值；否则返回一个索引为已指定的错误信息类型的指定连接错误信息
	 **/
	...
	/**
	 * @brief @~chinese 执行指定的SQL查询
	 * @param $query_string @~chinese 需要执行的SQL查询语句。如果需要同时执行多个SQL查询语句，每个SQL查询语句使用分号";"分隔
	 * @param $connect_id @~chinese 需要执行的目标连接ID。如果为空，则使用最近一次的连接ID
	 * @return @~chinese 如果成功则返回SQL查询ID；否则将错误信息标记为 E_USER_WARNING 并传递到错误处理句柄
	 **/
	...
	/**
	 * @brief @~chinese 获取指定SQL查询的结果
	 * @param $query_id @~chinese 需要获取的目标查询ID。如果唯恐，则使用最近一次的SQL查询ID
	 * @param $fetch_type @~chinese 需要返回的查询结果类型。
	 *                                                             @li ASSOC ＝ 仅返回以字段名作为索引名的结果
	 *                                                             @li NUM ＝ 仅返回以字段顺序号为索引名的结果
	 *                                                             @li BOTH ＝返回以字段名和字段顺序号为索引名的结果
	 * @return @~chinese 返回一个包括执行查询结果第一条记录的数组
	 * @warning @~chinese 本方法只能返回查询结果的第一条记录信息！如果需要返回指定查询的所有结果，请使用 fetchArrayBat()
	 * @note @~chinese 建议在使用本方法以后手动清除以保存的查询结果
	...
	function fetchArray($query_id = 0, $fetch_type = "BOTH")
	{
		...
		switch ($fetch_type)
		{
			case "ASSOC": $fetch_type_php = (("mysqli" != $this->mode) ? MYSQL_ASSOC : MYSQLI_ASSOC); break;
			case "NUM": $fetch_type_php = (("mysqli" != $this->mode) ? MYSQL_NUM : MYSQLI_NUM); break;
	...
	/**
	 * @brief @~chinese 获取指定SQL查询的所有结果
	 * @param $query_id @~chinese 需要获取的目标查询ID。如果为空，则使用最近一次的SQL查询ID
	 * @param $fetch_type @~chinese 需要返回的查询结果类型。
	 *                                                             @li ASSOC ＝ 仅返回以字段名作为索引名的结果
	 *                                                             @li NUM ＝ 仅返回以字段顺序号为索引名的结果
	 *                                                             @li BOTH ＝返回以字段名和字段顺序号为索引名的结果
	 * @return @~chinese 返回一个包括执行查询结果所有记录的数组
	 * @warning @~chinese 本方法会返回查询结果的所有记录信息！如果需要返回指定查询的第一条结果，请使用 fetchArray()
	 * @note @~chinese 强烈建议在使用本方法以后手动清除以保存的查询结果
	...
	/**
	 * @brief @~chinese 获得上次查询操作中自动生成的字段值
	 * @param $connect_id @~chinese 需要执行的目标连接ID。如果为空，则使用最近一次的连接ID
	 * @return @~chinese 如果成功则返回上次查询操作中自动生成的字段值，否则将错误信息标记为 E_USER_WARNING 并传递给错误处理句柄
	 **/
	...
	/**
	 * @brief @~chinese 获得指定数据表中的记录总数
	 * @param $table_name 需要查询的数据表名
	 * @param $connect_id @~chinese 需要执行的目标连接ID。如果为空，则使用最近一次的连接ID
	 * @param @~chinese 如果成功则返回指定数据库中的记录总数，否则将错误信息标记为 E_USER_WARNING 并传递给错误处理句柄
	 **/
	function getTotalCount($table_name, $connect_id = 0)
	{
		if (TRUE == empty($table_name)) return trigger_error($this->error_mesgs[7], E_USER_WARNING);
		$query_string = "select count(*) from " . $this->prefix . "$table_name";
		if (FALSE == is_resource($connect_id)) $connect_id = &$this->connect_id;
		if ("mysqli" != $this->mode)
		{
	...
	/**
	 * @brief @~chinese 获得指定SQL查询所影响的记录数
	 * @param $connect_id @~chinese 需要执行的目标连接ID。如果为空，则使用最近一次的连接ID
	 **/
	...
	/**
	 * @brief @~chinese 获得指定SQL SELECT查询获取的记录数
	 * @param $query_id @~chinese 需要获取的目标查询ID。如果为空，则使用最近一次的SQL查询ID
	 **/
	...
	/**
	 * @brief @~chinese 关闭指定的数据库连接
	 * @param $connect_id @~chinese  需要执行的目标连接ID。如果为空，则使用最近一次的连接ID
	 * @note @~chinese 本方法暂时对使用持续连接建立的数据库连接无效
	 **/
	...
	/**
	 * @brief @~chinese 释放指定连接上最后一次查询所使用的内存
	 * @param $query_id @~chinese 需要获取的目标查询ID。如果为空，则使用最近一次的SQL查询ID
	 * @note @~chinese 推荐在使用 fetchArrayBat() 以后立即使用本方法
	 * @see fetchArrayBat()
	 **/
