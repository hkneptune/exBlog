 ...
 * CVS Revision Version: $Revision: 1.20 $
 * Date Started: 2004/05/20
 * Last Modified: $Date: 2004/10/27 01:03:25 $
 *
...
class GeSHi
{
						...
						elseif ( $STRING_OPEN == '' )
						{
							...
							foreach ( $this->language_data['COMMENT_MULTI'] as $open => $close )
							{
								...
								if ( $open == $test_str )
								{
									...
									$rest_of_comment = htmlentities(substr($part, $i + $com_len, $close_pos - $i));
	...
	function add_url_to_keyword ( $keyword, $group, $start_or_end )
	{
		if ( isset($this->language_data['URLS'][$group]) && $this->language_data['URLS'][$group] != '' && substr($keyword, 0, 5) != '&lt;/' )
		{
			...
			if ( $start_or_end == 'BEGIN' )
			{
				...
				return '<|UR1|"' . str_replace(array('{FNAME}', '.'), array(htmlentities(strtolower($keyword)), '<DOT>'), $this->language_data['URLS'][$group]) . '">';
			}
			else
			{
				return '</a>';
	...
	function parse_non_string_part ( &$stuff_to_parse )
	{
		...
		if ( isset($this->link_styles[GESHI_LINK]) )
		{
			if ( $this->use_classes )
			{
				$stuff_to_parse = str_replace('<|UR1|', '<a href=', $stuff_to_parse);
			}
			else
			{
				$stuff_to_parse = str_replace('<|UR1|', '<a style="' . $this->link_styles[GESHI_LINK] . '" href=', $stuff_to_parse);
			}
		}
		else
		{
	...
	function finalise ( $parsed_code )
	{
		...
		if ( $this->enable_important_blocks && (strstr($parsed_code, htmlentities(GESHI_START_IMPORTANT)) === false) )
		{
			$parsed_code = str_replace(htmlentities(GESHI_END_IMPORTANT), '', $parsed_code);
		}
		...
		if ( $this->line_numbers != GESHI_NO_LINE_NUMBERS )
		{
			...
			foreach ( $code as $line )
			{
				...
				if ( $this->add_ids || in_array($i, $this->highlight_extra_lines) )
				{
	...
	function get_stylesheet ( $economy_mode = true )
	{
		...
		if ( !$economy_mode || count($this->highlight_extra_lines) )
		{
			...
			$stylesheet = substr($stylesheet, 0, strlen($stylesheet) - 1) . "{{$this->highlight_extra_lines_style}}\n";
		}
