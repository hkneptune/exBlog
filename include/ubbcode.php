...
class Coder
{
	...
	function addTag($pattern, $html = "", $is_pcre = TRUE)
	{
		if (TRUE == is_array($pattern))
		{
			for ($i = 0; $i < count($pattern); $i++)
			{
				if (FALSE == in_array($pattern[$i]['pat'], $this->patterns)) $this->patterns[] = array("search" => $pattern[$i]['pat'], "replace" => $pattern[$i]['html'], "pcre" => $pattern[$i]['pcre']);
			}
		}
		elseif (FALSE == in_array($pattern, $this->patterns)) $this->patterns[] = array("search" => $pattern, "replace" => $html, "pcre" => $is_pcre);
	}
	...
	function parseCode($str)
	{
		...
		{
			if (TRUE == $this->patterns[$i]['pcre']) $str = preg_replace($this->patterns[$i]['search'], $this->patterns[$i]['replace'], $str);
