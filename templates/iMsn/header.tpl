      ...
      <TABLE cellSpacing="0" cellPadding="0" width="100%" border="0">
        <TBODY>
          ...
          <TR>
            ...
            <TD align=right background="{$PHP_CONFIG.global.tmpURL}/images/tab.bg.sln.gif"><select name=template_name id=template_name onChange=changeQuery('template',this.options[selectedIndex].value)>{section name=template_id loop=$templates}<option value="{$templates[template_id].path}"{if $PHP_CONFIG.global.tmpName eq $templates[template_id].path} selected{/if}>{$templates[template_id].name}</option>{/section}</select></TD>
...
{if !$PHP_HAD_LOGIN}
            ...
            <tr>
              ...
              {if $PHP_CONFIG.global.GDswitch == 1}{$PHP_LANG.public[71]} <input name="imgVal" type="text" maxlength="5" class="input" size="5"> <img src="./include/VerifyImage.php?type=login&bgr=219&bgy=234&bgb=245&fr=16&fy=101&fb=158" align="absmiddle">{/if}
...
{else}
<tr>
  <td align=center>{$user_info.name} [<a href=./index.php?play=logout>{$PHP_LANG.index[30]}</a>]<br /><a href=./admin/editblog.php?action=add>{$PHP_LANG.public[77]}</a></td>
</tr>
{/if}
          </table>
        </td>
      ...
      <tr>
        <td bgcolor="#DBEAF5" class="content">
          <table width="100%">
            ...
            <tr><td><a class="con" href="./index.php?play=reply&id={$last_articles_list[last_blog].id}" title="{$last_articles_list[last_blog].title|escape:'html'}"> - {$last_articles_list[last_blog].summytitle|escape:"html"}</a></td></tr>
            {/section}
          </table>
        </td>
      </tr>
      ...
      <tr>
        <td bgcolor="#DBEAF5" class="content">
          <table width="100%">
            ...
            <tr><td><a class="con" href="./index.php?play=reply&id={$last_comments_list[last_comment].commentID}" title="{$last_comments_list[last_comment].content|escape:'html'}"> - {$last_comments_list[last_comment].summycontent|escape:'html'}</a></td></tr>
            {/section}
          </table>
        </td>
      </tr>
