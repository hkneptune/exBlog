...
<p align="center"><a href="{$smarty.server.PHP_SELF}?action=list&sortid={$article_info.sort}">{$PHP_LANG.wap[39]}</a><br /><a href="{$smarty.server.PHP_SELF}?action=viewcomment&id={$article_info.id}">{$PHP_LANG.wap[6]}</a><br /><a href="{$smarty.server.PHP_SELF}?action=addcomment&id={$article_info.id}">{$PHP_LANG.wap[7]}</a></p>
...
{elseif $action == 'addblog'}

<p align="center"><b>-{$PHP_LANG.wap[40]}-</b><br/></p>
<p>
{$PHP_LANG.wap[9]}<input name="name" type="text" /><br/>
{$PHP_LANG.wap[10]}<input name="password" type="text" /><br/>
{$PHP_LANG.wap[36]}<input name="subject" type="text" /><br/>
{$PHP_LANG.wap[42]}<input name="keyword" type="text" /><br />
{$PHP_LANG.wap[37]}<input name="trackback" type="text" emptyok="true" /><br/>
{$PHP_LANG.wap[38]}<input name="content" type="text" /><br/>
{$PHP_LANG.wap[15]}
</p>
<p align="center">
  <anchor title="submit">{$PHP_LANG.wap[16]}
    <go href="{$smarty.server.PHP_SELF}?action=addblog" method="post">
      <postfield name="name" value="$(name)" />
      <postfield name="password" value="$(password)" />
      <postfield name="subject" value="$(subject)" />
      <postfield name="trackback" value="$(trackback)" />
      <postfield name="esort" value="{$smarty.get.sortid}" />
      <postfield name="content" value="$(content)" />
    </go>
  </anchor>
</p>
<p align="center"><a href="{$smarty.server.PHP_SELF}?action=list&sortid={$smarty.get.sortid}">{$PHP_LANG.wap[19]}</a></p>
...
{/section}
	...
	{if $sort_id ne 0}<p align="center"><a href="{$smarty.server.PHP_SELF}?action=addblog&sortid={$smarty.get.sortid}">{$PHP_LANG.wap[41]}</a></p>{/if}
{/if}
    <p align="center">
      {$PHP_LANG.wap[1]}
    </p>
