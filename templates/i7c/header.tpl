...
{literal}
function changeQuery(query_name, value)
{
	query_string = location.search;
	reg_expression_1 = new RegExp("(.*?)"+query_name+"=(.*?)&(.*)","g");
	reg_expression_2 = new RegExp("(.*?)"+query_name+"=.*","g");
	if ('' == query_string) query_string = "?" + query_name + "=" + value;
	else if (reg_expression_1.test(query_string)) query_string = query_string.replace(reg_expression_1, "$1"+query_name+"=" + value +"&$3");
	else if (reg_expression_2.test(query_string)) query_string = query_string.replace(reg_expression_2, "$1"+query_name+"=" + value);
	else query_string += "&" + query_name + "=" + value;
...
<select name=template_name id=template_name onChange=changeQuery('template',this.options[selectedIndex].value)>{section name=template_id loop=$templates}<option value="{$templates[template_id].path}"{if $PHP_CONFIG.global.tmpName eq $templates[template_id].path} selected{/if}>{$templates[template_id].name}</option>{/section}</select>
...
{if $PHP_CONFIG.global.GDswitch == 1}{$PHP_LANG.public[71]} <input name="imgVal" type="text" maxlength="5" class="input" size="5"> <img src="./include/VerifyImage.php?type=login&bgr=255&bgy=255&bgb=255&fr=0&fy=0&fb=0" align="absmiddle"><br />{/if}
...
{else}
{$user_info.name} [<a href=./index.php?play=logout>{$PHP_LANG.index[30]}</a>]<br /><a href=./admin/editblog.php?action=add>{$PHP_LANG.public[77]}</a>
{/if}
</div>
<br>
...
{section name=last_blog loop=$last_articles_list}
            <tr><td><img src="./{$PHP_CONFIG.global.tmpURL}/images/arrow.gif" border="0"> <a href="./index.php?play=reply&id={$last_articles_list[last_blog].id}" title="{$last_articles_list[last_blog].title|escape:'html'}">{$last_articles_list[last_blog].summytitle|escape:'html'}</a></td></tr>
            {/section}
                              </table></td>
                            </tr>
                          </table>
...
{section name=last_comment loop=$last_comments_list}
            <tr><td><img src="./{$PHP_CONFIG.global.tmpURL}/images/arrow.gif" border="0"> <a href="./index.php?play=reply&id={$last_comments_list[last_comment].commentID}" title="{$last_comments_list[last_comment].content|escape:'html'}">{$last_comments_list[last_comment].summycontent|escape:'html'}</a></td></tr>
            {/section}
                              </table></td>
                            </tr>
                          </table>
...
{/section}
          ...
          <a href="./rss.php?version=atom" target="_blank"><img src="{$PHP_CONFIG.global.tmpURL}/images/atom03.png" alt="ATOM View v:0.3" border="0"></a>
