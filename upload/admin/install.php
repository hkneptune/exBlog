...
$GLOBALS[exBlogVer] = "exBlog Ver: 1.3.final";	//当前版本 显示用
$GLOBALS[exBlogVer_s] = "exBlog 1.3.final";	//当前版本 检测用(用户千万不要修改)
...
if($_POST[action] ==""||$_POST[action] == "step_welcome")
{
    step_welcome();
...
function step_db()
{
 ...
 if (@file_exists('../include/config.inc.php')){
        ...
        if ($fp=@fopen('../include/config.inc.php',"w")) {
              @fclose($fp); @unlink('../include/config.inc.php');
        } else { $checkpath=0; }
 }
...
function step_create_db()
{
	...
	$dbname=$_POST[dbname];
	$one=$_POST[one];
	$mySQL_create=$_POST[mySQL_create];
    if ($mySQL_create == 1) {
...
"<" . "?php
######### exBlog网络日志相关设置 #########
	\$exBlog['host'] = \"" . $host . "\";		//数据库地址
	\$exBlog['user'] = \"" . $user . "\";		//数据库用户名
	\$exBlog['password'] = \"" . $password . "\";	//数据库密码
	\$exBlog['dbname'] = \"" . $dbname . "\";	//数据库名
	\$exBlog['one'] = \"" . $one . "\";		//新建数据表名前缀
      ...
      <td bgcolor="#FFFFFF"> 
        <table width="100%" border="0" class="main">
          ...
          <tr>
            ...
            <td><input name="exBlogCopyright" type="text" class="input" value="Copyright &copy; 2003-2005 &lt;a href=http://www.exblog.net target=_blank class=copy&gt;exBlog&lt;/a&gt; All Rights Reserved."></td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td><input name="templatesURL" type="text" class="input" value="templates/iMsn"></td>
          </tr>
          <tr>
