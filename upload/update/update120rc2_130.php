<?php
/**
 *  1.2.0_RC2  [to] exBlog 1.3.0 升级程序
 *  2004-02-12
 */
...
function alterGlobal()
{
	...
	$query  = "ALTER TABLE `$x[global]` CHANGE `Version` `Version` VARCHAR( 20 ) DEFAULT 'exBlog 1.3.0'"; //为了版本检测请按这个标准写这个: 'exBlog 1.3.0'
    @mysql_query($query)
    ? $result.="更改表结构 `$x[global]` 成功!...... <font color='blue'>√</font><br />"
    : $result.="更改表结构 `$x[global]` 失败! ...... <font color='red'>×</font><br />";

	$query  = "UPDATE `$x[global]` SET Version='exBlog 1.3.0'"; //为了版本检测请按这个标准写这个: 'exBlog 1.3.0'
    @mysql_query($query)
...
{
	global $x;
$querysort="SELECT * FROM `$x[sort]` ORDER BY id DESC";
$resultSort=@mysql_query($querysort) or die("调整 文章分类 失败!");
	while($row=mysql_fetch_array($resultSort))
	{
	$query2="UPDATE `$x[blog]` SET sort='$row[id]' WHERE sort='$row[cnName]'";
	@mysql_query($query2) or die("调整 文章分类 失败!");
	$query3="UPDATE `$x[comment]` SET commentSort='$row[id]' WHERE commentSort='$row[cnName]'";
	@mysql_query($query3) or die("调整 文章分类 失败!");
	$gogo.="调整 $row[cnName] 分类 成功!";
	}
