<?php
/**
 *  1.2.0_RC2  [to] exBlog 1.3.final 升级程序
 *  2004-05-02
 */
...
function alterGlobal()
{
	...
	$query  = "ALTER TABLE `$x[global]` CHANGE `Version` `Version` VARCHAR( 20 ) DEFAULT 'exBlog 1.3.final'"; //为了版本检测请按这个标准写这个: 'exBlog 1.3.0'
    @mysql_query($query)
    ? $result.="更改表结构 `$x[global]` 成功!...... <font color='blue'>√</font><br />"
    : $result.="更改表结构 `$x[global]` 失败! ...... <font color='red'>×</font><br />";

	$query  = "UPDATE `$x[global]` SET Version='exBlog 1.3.final'"; //为了版本检测请按这个标准写这个: 'exBlog 1.3.0'
    @mysql_query($query)
