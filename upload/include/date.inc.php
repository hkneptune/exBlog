...
function selectCurrentYearMonthBlog($iYear,$iMonth)
{
    ...
    if($num == 0)
    {
		    $t->set_var(array(	"blogID"=>"0",
					"blogTitle"=>$langfun[45],
					"url_reply"=>"",
					"url_show"=>"",
					"blogContent"=>$langfun[45],
					"blogSort"=>"NULL",
					"blogSortid"=>"0",
...
# 词汇解释 结束
				...
				$rows['addtime']=eregi_replace($iTime,"<span style=\"color:#f00;font-weight:bold;text-decoration:underline;background-color: #F0F0F0;\">$iTime</span>","$rows[addtime]");
...
{
	global $t, $x, $prev_page, $next_page, $total_page, $info, $langfun;
$exubb = new ExUbb();
	$page_size=$info[listblognum];
	...
	while($rows=mysql_fetch_array($query))
	{
		...
		if($hidden_on == "1")
		  {   
			...
			$today_blogAddtime[$i]="0000-00-00 00:00";
			$sort[$i]="$langfun[22]";
			$blog_visits[$i]="0";
...
# 词汇解释 结束
	        $rows['content']=epost($rows['content']);
			$rows['addtime']=eregi_replace($iDate,"<span style=\"color:#f00;font-weight:bold;text-decoration:underline;background-color: #F0F0F0;\">$iDate</span>",$rows['addtime']);
...
function pigeonhole()
{
    ...
    $query="SELECT DISTINCT addtime FROM `$x[blog]` ORDER BY addtime DESC";
    $result=mysql_query($query);
    $num=mysql_num_rows($result);
    if($num == 0)
    {
  	$t->set_var("pigeonhole", $langfun[55]);
  	$t->set_var("url_pigeonhole", "");
  	$t->parse("m_pigeonhole", "RowsPigeonhole", true);
    }else{
        $pigeonhole = array();
        $is_go = 1;
	    ...
	    $date_gd=gmdate("M,Y", $timestamp);
	    $is = 0;
	    foreach($pigeonhole as $list_key=>$list_value) {
		    list($pigeonhole_show, $pigeonhole_year, $pigeonhole_month)=explode("||",$list_value);
		    if( $date_gd == $pigeonhole_show ) $is = 1;
		    $is_go = 0;
	    }
	    if( $is == 0 || $is_go == 1 ) $pigeonhole[] = $date_gd."||".$at_year."||".$at_month;
        }
        foreach($pigeonhole as $list_key=>$list_value) {
	    list($pigeonhole_show, $pigeonhole_year, $pigeonhole_month)=explode("||",$list_value);
	    if ($exurlon) {
	        $url_pigeonhole=$GLOBALS[expath]."/exurl.php/calendar/".intval($pigeonhole_year)."/".intval($pigeonhole_month).".html";
	    }else{
	        $url_pigeonhole=$GLOBALS[expath]."/index.php?play=calendar&amp;year=".intval($pigeonhole_year)."&amp;month=".intval($pigeonhole_month);
	    }
  	    $t->set_var("pigeonhole", $pigeonhole_show);
  	    $t->set_var("url_pigeonhole", $url_pigeonhole);
  	    $t->parse("m_pigeonhole", "RowsPigeonhole", true);
        }
    }
}
