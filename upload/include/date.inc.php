...
function selectCurrentYearMonthBlog($iYear,$iMonth)
{
    ...
    if($num == 0)
    {
	    $t->set_var(array("blogTitle"=>"该月目前无任何文章","currentSort"=>$sortName[cnName],"blogWeather" => "./images/weather/null.gif"));
	...
	while($rows=mysql_fetch_array($query))
	{
		if($i<$num)
		{
		  ...
		  {   
			...
			$today_blogTitle[$i]="该文章作者隐藏";
			$today_blogAuthor[$i]="隐藏";
			$today_blogAddtime[$i]="隐藏";
			$today_blogSort[$i]="隐藏";
			$blog_visits[$i]="隐藏";
			$today_blogWeather[$i]="null";
			$today_blogReply[$i]="隐藏";
		  }
			else
		  {
			$rows['content']=substr($rows['content'],"0","450").chr(0);
