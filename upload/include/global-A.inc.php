...
function checkunregist()
{
	...
	session_start();
	if(session_is_registered("exPass") && $_COOKIE['exBlogUserid']!="" && $_COOKIE['exBlogUserpassword']!="")
	{ 
		return false;
	}
	else
...
function addComment($iCommentID, $iCommentSort, $iAuthor, $iEmail, $iHomepage, $iQQ, $iContent, $password, $register)
{
 ...
 if (!checkunregist()) {
      $iAuthor=$_COOKIE['exBlogUserid'];
      $password="111111";
      $iEmail=$_COOKIE['exBlogUseremail'];
	...
	if($password == "")
	{
		...
		$query3 = "SELECT user FROM `$x[admin]` WHERE user = '$iAuthor'";
		//$result2 = mysql_query($query2);
		$result3 = mysql_query($query3);
           $result=0;
           //$result=mysql_num_rows($result2)+mysql_num_rows($result3);
           $result=mysql_num_rows($result3);
		if( $result )
			showError("$langfun[10]");
	}
	else if($password != "")
	{
 if (!checkunregist()) {
		$passwd=$_COOKIE['exBlogUserpassword'];
 }else{
		$passwd = md5($password);
 }
		//$query2 = "SELECT * FROM `$x[user]` WHERE name = '$iAuthor' AND password = '$passwd'";
		$query3 = "SELECT * FROM `$x[admin]` WHERE user = '$iAuthor' AND password = '$passwd'";
		//$result2 = mysql_query($query2);
		$result3 = mysql_query($query3);
           $result=0;
           //$result=mysql_num_rows($result2)+mysql_num_rows($result3);
...
if ($exurlon) {
    ...
    $url_regLink=$GLOBALS[expath]."/index.php?play=regLink";
  	  ...
  	  $t->set_var(array("title"=>$info[siteName],
			...
			"Webmastername"=>$info2[name],
			"Webmasteremail"=>$info2[email],
			"Webmasterage"=>$info2[age],
			"WebmasterQQ"=>$info2[qq],
			"Webmastericq"=>$info2[icq],
			"Webmastermsn"=>$info2[msn],
			"Webmasterdescription"=>$info2[description],
			"sitedescription"=>$info[Description],
			"sitekeyword"=>$info[sitekeyword],
			"url_links"=>$url_links,
...
function shell($iSort)
{
	...
	{
		$query = "SELECT * FROM `$x[blog]` WHERE hidden='0' sort='$iSort' ORDER BY id DESC LIMIT 0,8";
	}
...
function selectAnnounce()
{
	...
	while($rows=mysql_fetch_array($sql))	//while($db->next_record())
	{
		...
		$announcetitle[$i]=clearUbb2($announcetitle[$i]);
  	$exubb = new ExUbb();
	$exubb->setString($announcecontent[$i]);
		$announcecontent[$i] = $exubb->parse();
		$announcetitle[$i]=epost($announcetitle[$i]);
########################################
		//---- 把标题截取18个字符 ----//
...
########################################
		...
		$t->set_var(array("announceid"=>$announceid[$i],"announcetitle"=>$announcetitle[$i],
...
function putAnnounce($iID)
{
	...
	$rows['title'] = clearUbb2($rows['title']);
	...
	$rows['content'] = clearUbb2($rows['content']);
...
function searchInfo($key)
{
	...
	$sql=mysql_query("SELECT * FROM `$x[blog]`  WHERE hidden='0' content LIKE '%$key%' OR title LIKE '%$key%'");
	$num=mysql_num_rows($sql);
	if($num == "0")
	{
		showError("$langfun[16]");
	}
...
function selectNewblog()
{
	...
	while($rows=mysql_fetch_array($sql))
	{
		...
		$newblogtitle[$i]=clearUbb2($newblogtitle[$i]);
...
if(checkunregist()&&$tmpBlog['hidden']=="1")
	{   $t->set_var(array("blogTitle"=>"$langfun[21]",
				"currentSort"=>"$langfun[21]",
				"blogAuthor"=>"$langfun[22]",
				"blogSort"=>"$langfun[22]",
				"blogAddtime"=>"$langfun[22]",
				"blogID"=>$curID,
...
$sort=$resultSort['cnName'];
	...
	$tmpBlog['title']=clearUbb2($tmpBlog['title']);
...
# 词汇解释 结束
	$tmpBlog['content'] = clearUbb2($tmpBlog['content']);
   ...
   if (!checkunregist()) {
		...
		$t->set_var("exBlogUserid", $_COOKIE['exBlogUserid']);
    }else{
...
function showCurrentBlogComment($curID)
{
	...
	$rows['content']=epost($rows['content']);
	$rows['content'] = clearUbb2($rows['content']);
	$rows['author'] = clearUbb2($rows['author']);
	$rows['email'] = clearUbb2($rows['email']);
	$rows['homepage'] = clearUbb2($rows['homepage']);
	$rows['qq'] = clearUbb2($rows['cqq']);
...
###### 显示当前BLOG的引用 ######
function showCurrentTrackback($curID)
{
	global $t, $x, $langfun;
	...
	$rows['excerpt'] = clearUbb2($rows['excerpt']);
	$rows['title'] = clearUbb2($rows['title']);
	$rows['blog_name'] = clearUbb2($rows['blog_name']);
	$rows['url'] = clearUbb2($rows['url']);
...
function selectCurSortArt($isort)
{
    ...
    if($num == 0)
    {
	    $t->set_var(array("blogTitle"=>"$langfun[25]","currentSort"=>$sortName[cnName],"blogWeather" => "$GLOBALS[expath]/images/weather/null.gif"));
...
# 词汇解释 结束
	...
	$rows_top['title'] = clearUbb2($rows_top['title']);
	$rows_top['content'] = clearUbb2($rows_top['content']);
	$rows_top['author'] = clearUbb2($rows_top['author']);
	$rows_top['email'] = clearUbb2($rows_top['email']);
	$rows_top['sort'] = clearUbb2($rows_top['sort']);
...
if ($exurlon) {
  ...
  "url_show"=>$url_show,
    ...
    if($num == 0)
    {
           $t->set_var(array("blogTitle" => "$langfun[28]","blogWeather" => "$GLOBALS[expath]/images/weather/null.gif"));
	    $t->parse("m_blog", "RowsBlog", true);
    }
    else
    {
...
function addUser($name, $password, $email, $homepage, $qq)
{
	...
	//$query2 = "SELECT name FROM `$x[user]` WHERE name = '$name'";
    ...
    userLogin($name, $password, 1, 1, "no");
...
function highlight($str,$key) {
	return(preg_replace("/([^<]*)($key)([^>]*)($|<.+)/isU","$1<span style=\"color:#f00;font-weight:bold;text-decoration:underline\">$2</span>$3$4",$str));
}

function acronym($str,$a_word,$a_content,$a_url) {
    if(!$a_url){
	return(preg_replace("/([^<]*)($a_word)([^>]*)($|<.+)/isU","$1<acronym title=\"$a_word: $a_content\">$a_word</acronym>$3$4",$str));
    }else{
	return(preg_replace("/([^<]*)($a_word)([^>]*)($|<.+)/isU","$1<a href=\"$a_url\" target=\"_blank\"><acronym title=\"$a_word: $a_content\">$a_word</acronym></a>$3$4",$str));
    }
}
