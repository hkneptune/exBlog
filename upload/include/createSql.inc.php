...
function CreateLinks($i)
{
	...
	$query.=" VALUES('exBlog\'s Home','./images/logo_s.gif','http://www.exBlog.net','exBlog 官方主页', '0', '0')";
...
function CreateAboutme($i)
{
    ...
    $query2.=" VALUES('Name','20','@','','','','欢迎使用exBlog程序,如何任何问题请在主页或来信提出,谢谢!!')";
...
function CreatePhoto($i)
{
    ...
    $query2.=" VALUES('2000000', 'upload/', '0', '2', 'http://www.fengling.net/')";
    @mysql_query($query2) or die("初始化 `$i_photo` 失败 ...... <font color='red'>X</font><br>");
    $result.="初始化 `$i_photo` 成功...... <font color='red'>√</font><br>";
}
/* 创建系统管理员 */
function CreateSystem($iUser, $iPassword, $iPassword_2, $iEmail, $iPhone, $i)
{
	$i_admin="$i"."admin";
...
function CreateGlobal($i)
{
	...
	$query.="Version VARCHAR( 20 ) DEFAULT 'exBlog 1.2.0 [L]' ,";
	$query .= "Webmaster VARCHAR( 35 ) DEFAULT 'exblog@fengling.net' NOT NULL)";
    @mysql_query($query) or die("创建 `$i_global` 失败 ...... <font color='red'>X</font><br>");
    $result.="创建表 `$i_global` 成功...... <font color='red'>√</font><br>";
	
    $query="INSERT INTO `$i_global` (siteName, siteUrl, copyright, tmpURL, activeRun, unactiveRunMessage, initTime, isCountOnlineUser, Description)";
    $query.=" VALUES('exBlog ver:1.2.0 [L] 圣诞版', 'http://www.exBlog.net','Copyright &copy; 2004-2005 <a href=\"http://www.exblog.net\" target=\"_blank\" class=\"copy\">exBlog ver: 1.2.0 [L] 圣诞版</a> All Rights Reserved.',";
	$query.="'templates/xmas', '1', '由于XXX原因,本站暂时关闭X天.','$nowTime', '0', '本Blog由exBlog技术支持')";
