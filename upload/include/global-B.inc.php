...
function updateGlobal($iSiteName,$iSiteUrl,$iCopyright,$iTmpURL, $activeRun, $unactiveRunMessage, $isCountOnlineUser)
{
	global $x;
	...
	$qUpdateGlobal .= "unactiveRunMessage = '$unactiveRunMessage', isCountOnlineUser = '$isCountOnlineUser'";
...
/* update blog content */
function updateBlog($title, $content, $author, $email, $sort, $ID, $addtime, $weather, $top)
{
	global $x;
	...
	$qUpdateArt.="author='$author', email='$email', sort='$sort', addtime='$addtime', weather = '$weather', top = '$top' WHERE id='$ID'";
...
function updateSort($iCnName,$iEnName,$iDescription,$iOldCnName,$iID)
{
	...
	$query="SELECT * FROM `$x[sort]` WHERE enName='$iEnName' AND cnName='$iCnName'";
    $result=mysql_num_rows(mysql_query($query));
    if($result > 0)
	{
         showError("该分类名在数据库中已存在!");
...
function updateSortrank($ioldID,$inewID)
{
    ...
    //-----  被替换分类暂时移位 -------//
	$query="UPDATE `$x[sort]` SET `id` = '10000' WHERE `id` = '$inewID' LIMIT 1 ";
	@mysql_query($query) or die("更新所选分类时失败!");
	...
	@mysql_query($query2) or die("更新原有文章分类时失败!");
	//----- 还原被替换分类到新位置 ------//
	$query3="UPDATE `$x[sort]` SET `id` = '$ioldID' WHERE `id` = '10000' LIMIT 1";
...
function updateLinkrank($ioldID,$inewID)
{
    ...
    //-----  被替换链接暂时移位 -------//
	$query="UPDATE `$x[links]` SET `id` = '10000' WHERE `id` = '$inewID' LIMIT 1 ";
	...
	//----- 还原被替换链接到新位置 ------//
	$query3="UPDATE `$x[links]` SET `id` = '$ioldID' WHERE `id` = '10000' LIMIT 1";
 ...
 */
function addBlog($title, $content, $author, $email, $sort, $weather, $top)
{
	global $x;
    ...
    $query="INSERT INTO `$x[blog]` (title, content, author, email, sort, visits, addtime, weather, top)";
    $query.=" VALUES('$title', '$content', '$author', '$email', '$sort', '0', now(), '$weather', '$top')";
    @mysql_query($query) or die(mysql_error());
    $showMsg="添加文章成功!";
...
/* 添加新用户 */
function addNewUser($iUser, $iPassword, $iPassword_2, $iEmail, $iUid)
{
	global $x;
	...
	$query.="VALUES('$iUid','$iUser','$iPassword','$iEmail')";
...
/* 更新所选用户操作 */
function updateOneObj($iUser, $iPassword, $iPassword_2, $iEmail, $iUid, $iID)
{
	global $x;
	$result=checkUser($iUser);
	...
	$query="UPDATE `$x[admin]` SET user='$iUser', $param email='$iEmail',";
...
function selectAllWeather()
{
    ...
    $resultWeather=mysql_query($query) or die("查询所有分类时失败!");
}
