...
function editUserSelUid($curUid, $UserUid)
{
	if($curUid == $UserUid)
	{
		echo "selected";
...
/* update blog content */
function updateBlog($title, $content, $author, $email, $sort, $ID, $addtime, $keyword, $summarycontent, $weather, $top, $hidden, $html, $nocom)
{
	global $x, $langadfun;
	...
	$qUpdateArt.="author='$author', email='$email', sort='$sort', addtime='$addtime', keyword='$keyword', summarycontent='$summarycontent', weather = '$weather', top = '$top', hidden = '$hidden', html = '$html', nocom = '$nocom' WHERE id='$ID'";
 ...
 */
function addBlog($title, $content, $author, $email, $sort, $addtime, $keyword, $summarycontent, $weather, $top, $hidden, $html, $nocom, $trackback)
{
	global $x, $langadfun;
...
########################
	...
	if(!$addtime){
		...
		$query="INSERT INTO `$x[blog]` (title, content, author, email, sort, visits, addtime, keyword, summarycontent, weather, top, hidden, html, nocom)";
		$query.=" VALUES('$title', '$content', '$author', '$email', '$sort', '0', '$addtime', '$keyword', '$summarycontent', '$weather', '$top', '$hidden', '$html', '$nocom')";
		@mysql_query($query) or die(mysql_error());
...
/* edit blog page */
function selectBlogModify($s_addtime,$s_sort,$s_title,$s_content,$s_px,$s_sc,$s_author)
{
...
if (!$s_px) $s_px="id";
	$page_size=30;
    $queryBlog="SELECT * FROM `$x[blog]` WHERE $sq_addtime $sq_sort $sq_title $sq_content $sq_author 1=1 ORDER BY $s_px $s_sc";
    ...
    $query="SELECT * FROM `$x[blog]` WHERE $sq_addtime $sq_sort $sq_title $sq_content $sq_author 1=1 ORDER BY $s_px $s_sc limit $start,$page_size";
...
function editBlogSelNocom($curNocom)
{
    global $langadfun;
	if($curNocom == 0)
	{
